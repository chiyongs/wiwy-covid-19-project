...
queryParams +=
  ...
  "=" +
  encodeURIComponent("20201101"); /**/
queryParams +=
  "&" +
  encodeURIComponent("endCreateDt") +
  "=" +
  encodeURIComponent("20201130"); /**/
...
xhr.onreadystatechange = function () {
  if (this.readyState == 4) {
    ...
    // console.log(objResult.response.body.items.item.length);
    for (let i = 0; i < objResult.response.body.items.item.length; i++) {
      ...
      // console.log(typeof objResult.response.body.items.item[i].defCnt._text);
      // INSERT QUERY
      connection.query(
        `INSERT INTO Covid202010(seq,gubun,defCnt,incDec,stdDay,gubunCn,gubunEn,qurRate,createDt,deathCnt,updateDt,isolIngCnt,localOccCnt,overFlowCnt,isolClearCnt)
    ...
    // fs.writeFile("test1.json", stringResult, "utf8", (err) => {
    //   if (err) throw err;
    // });
  }
};
